...
def make_windowed_data(data, start, end, window_size=1):
    ...
    for sentence, labels in data:
        win = [start] * (window_size * 2 + 1)
        if window_size > (len(sentence) - 1):
            win[window_size:] = [end] * (window_size + 1)
            win[window_size: window_size + len(sentence)] = sentence
        else:
            win[window_size:] = sentence[:window_size + 1]
        windowed_data.append((unlist(win), labels[0]))

        for i in range(1, len(sentence)):
            win[:-1] = win[1:]
            if i + window_size < len(sentence):
                win[-1] = sentence[i + window_size]
            else:
                win[-1] = end
            windowed_data.append((unlist(win), labels[i]))
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(
            tf.int32,
            (None, self.config.n_window_features),
            "input"
        )
    ...
    def add_embedding(self):
        ...
        """
        out_shape = (-1, self.config.n_window_features * self.config.embed_size)

        embeddings = tf.Variable(self.pretrained_embeddings)
        embedded_input = tf.nn.embedding_lookup(embeddings,
                                                self.input_placeholder)
        embedded_input = tf.reshape(embedded_input, out_shape)
    ...
    def add_prediction_op(self):
        ...
        """
        input_size = self.config.n_window_features * self.config.embed_size
        ...
        self.b1 = tf.get_variable("b1", (1, self.config.hidden_size), tf.float32,
                                  tf.zeros_initializer())
        self.b2 = tf.get_variable("b2", (1, self.config.n_classes), tf.float32,
                                  tf.zeros_initializer())

        h = tf.nn.relu(tf.matmul(x, self.W) + self.b1, "hidden_layer")
        h_drop = tf.nn.dropout(h, 1 - self.dropout_placeholder)
        pred = tf.nn.relu(tf.matmul(h_drop, self.U) + self.b2, "output_layer")
