...
def minibatch_parse(sentences, model, batch_size):
    ...
    partial_parses = map(PartialParse, sentences)
    ...
    while len(unfinished_parses):
        ...
        for i in range(0, len(minibatch)):
            ...
            if not len(minibatch[i].buffer) and len(minibatch[i].stack) == 1:
                unfinished_parses.remove(minibatch[i])
    return map(lambda x: x.dependencies, partial_parses)
