...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    sentWordVectors = np.take(wordVectors, [tokens[word] for word in sentence])
    sentVector = np.average(sentWordVectors, axis=0)
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    prob[range(N), labels] -= 1.0
    grad = 1.0 / N * np.dot(features.T, prob) + regularization * weights
